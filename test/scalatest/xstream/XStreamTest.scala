...
object XStreamTest {
  ...
  def main(args: Array[String]) {
    val stream = new XStreamUtils(Array(classOf[WeixinConfig],classOf[WeixinAppInfo],classOf[TodoConfig],classOf[ProcessConfig],classOf[Process],classOf[Condition]))
    ...
    println(weixinConfig.getProcessConfig.process(0).conditions(0).getName)
  }
}
